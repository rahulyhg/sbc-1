...
<div class="container">
		...
		<div class="container1">
			<div class="generic_left col-xs-12">
				 ...
				 <div class="nav_content1 title_icon title_wide icon-program-{{ Session::get('program_id') }}">
					 <h1 class="icon-lg-program-{{ Session::get('program_id') }}">Weight Chart</h1>
				 </div>
				 
				<div class="col-md-8 col-xs-12">
					<a id='week' class="btn btn-info">Weekly</a>
					<a id='month' class="btn btn-info">Monthly</a>
					{{--<a id='year' class="btn btn-info">Year</a>--}}
				</div>
    ...
    <div id='' style="width:100%;">
       ...
       <!-- <div class="main_goal"><p class="weight_goal">
           My Weight Goal: @if($weight_unit == 'lb')
            {{round(convert_to_lb($weight_goal),2)}} {{$weight_unit}}
           @else
            {{round($weight_goal,1)}} {{$weight_unit}}
           @endif
       </P></div> -->
    </div>
...
var weight_unit=$('#weight_unit').val();
            ...
            {{--var startDate = moment($.now()).startOf('month').format('YYYY-MM-DD');--}}
            {{--var endDate = moment($.now()).endOf('month').format('YYYY-MM-DD');--}}
        ...
        <div class="col-md-12">
            ...
            <ul class="list-group">
                @foreach ($measurements as $measurement)
                    <li class="list-group-item">{{date('M d, Y',strtotime($measurement->date))}}
                        <span class="pull-right">
                            @if($weight_unit == 'lb')
                                {{round(convert_to_lb($measurement->weight),2)}} {{$weight_unit}}
                            @else
                                 {{round($measurement->weight,1)}} {{$weight_unit}}
                            @endif
                        </span>
                        </li>
                @endforeach
                ...
                {{date('M d, Y',strtotime($created_at))}}
                        <span class="pull-right">
                            @if($weight_unit == 'lb')
                             {{round(convert_to_lb($weight),2)}} {{$weight_unit}}
                            @else
                             {{round($weight,1)}} {{$x_unit}}
                            @endif
                        </span>
                    </li>
            </ul>
